...
.left[.footnote[.black[
Pagina creada por mi equipo - [Formato FASTQ en Wikipedia](https://es.wikipedia.org/wiki/Formato_FASTQ)
]]]
...
- Obtener información sobre la complejidad de la biblioteca (contaminación por rRNA, duplicaciones)
...
## Tarea para el **Jueves 29 de Feb**
...
Equipos de 3 personas (3 equipos de 3 personas y un equipo de 2)
...
.content-box-red[
  - **Especie modelo** (*Homo sapiens, Mus musculus, Drosophila melanogaster o Arabidopsis thaliana*).
  ...
  - Contar con un **1 contraste como mínimo** (ejemplo: control vs condición 1).
  - Contar con una buena **calidad de los datos** (FastQC).
]

---

### 3) Deben entregarme un documento en hmtl y Rmd con la **Descripción de los datos** por equipo:
...
Enviar al correo ecoss@liigh.unam.mx. 
...
Cuando terminen tendrán la siguiente distribución en sus carpetas. El directorio donde estamos localizados es `Clase_RNASeq2024/`.

```{bash, eval=F}
Clase_RNASeq2024/ # directorio de trabajo donde nos localizamos
|- data/
|- counts/
```
...
```{bash, eval=F}
rsync ecoss@liigh.unam.mx:"/mnt/Guanina/bioinfo24/data/Clase_RNASeq2024/test.txt" .
```

Subir Información

```{bash, eval=F}
rsync ./test.txt ecoss@liigh.unam.mx:"/mnt/Guanina/bioinfo24/data/Clase_RNASeq2024/"
```
